...
vector<PolyNode> Difference(vector<PolyNode> &pa) {
    vector<PolyNode> ret;
    for (vector<PolyNode>::iterator iter = pa.begin(); iter != pa.end();
         ++iter) {
        if (iter->exp == 0) // 指数为0
            continue;
    ...
    sort(ret.begin(), ret.end(),
         [](const PolyNode &a, const PolyNode &b) { return a.exp > b.exp; });
    return ret;
}
...
void split(const string &s, vector<string> &tokens, const string &delim = " ") {
    ...
    size_t pos = s.find_first_of(
        delim, lastpos); // 从lastpos开始查询，第一个分隔符的位置
...
int main() {
    ...
    for (auto item : item_vec) {
        auto x_pos = find_if(item.begin(), item.end(),
                             [](const char ch) { return ch == 'x'; });
        PolyNode node;
    ...
    string output;
    for (vector<PolyNode>::iterator iter = out.begin(); iter != out.end();
         ++iter) {
        string item;
        // 处理系数
